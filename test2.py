...
for train_idx, val_idx in kfold.split(X_mi_val, y_val):
    ...
    # 
    model_dir = 'model'
    model_path = os.path.join(model_dir, '11-11.h5')
    ...
    # 
    print(
        f" (Accuracy): {accuracy:.4f},  (F1 Score): {f1:.4f}, ROC AUC : {roc_auc:.4f}, AUPR: {aupr:.4f}, MCC: {mcc:.4f},  (Sensitivity): {sensitivity:.4f},  (Precision): {precision:.4f},  (Specificity): {specificity:.4f}")
...
print(f"AUPR: {np.mean(aupr_scores):.4f}")
